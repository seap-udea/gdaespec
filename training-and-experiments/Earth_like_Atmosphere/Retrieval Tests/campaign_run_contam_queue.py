#!/usr/bin/env python3
"""Run missing Contam + Chem campaign retrievals sequentially.
        ...
        for branch in ("phoenix", "sphinx"):
            ...
            for case in iter_cases(branch):
                ...
                if f_fac_filter is not None and abs(case.f_fac - f_fac_filter) > 1.0e-9:
                    continue
                if result_exists(test_id, branch, case.model_name("contam")):
                    continue
...
def run_job(job: tuple[str, str, float, float], nproc: int, dry_run: bool) -> int:
    """Launch one contamination-retrieval job with mpirun."""
    test_id, branch, f_spot, f_fac = job
    log_dir = CAMPAIGN_DIR / "logs"
    log_dir.mkdir(parents=True, exist_ok=True)
    stamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    log_path = log_dir / f"{stamp}_{test_id}_{branch}_contam_{f_spot:.2f}spot-{f_fac:.2f}fac.log"
    cmd = [
        "mpirun",
        "-n",
        str(nproc),
...
def main() -> int:
    ...
    print(f"Queued Contam + Chem jobs: {len(jobs)}")
    for job in jobs:
